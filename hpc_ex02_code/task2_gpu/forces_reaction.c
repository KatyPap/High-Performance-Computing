...
void computeGravitationalForces(Particle_t *particles, int n){
	...
	for (int i=0; i<n; i++){
      ...
      particles[j].fx += (particles[j].x-particles[i].x) * magnitude;
      particles[j].fy += (particles[j].y-particles[i].y) * magnitude;
      particles[j].fz += (particles[j].z-particles[i].z) * magnitude;
		}
	}
}
