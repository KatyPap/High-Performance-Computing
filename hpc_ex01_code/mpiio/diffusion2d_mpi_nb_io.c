...
void write_density_mpi(Diffusion2D *D2D, char *filename)
{
    ...
    // All density data to be written except the ghost cells
    double *all_data = (double *)malloc(local_N_ * N_ * sizeof(double));
    for (int i = 1; i <= local_N_; ++i) {
        for (int j = 1; j <= N_; ++j) {
            all_data[(i - 1) * N_ + (j - 1)] = rho_[i * real_N_ + j];
        }
    }

    // Each process needs to know how it is going to write in the file:
    // It needs to know how much data it is going to write 
    MPI_Offset len = local_N_ * N_ * sizeof(double);
    // It also needs to know where in the file to write
    MPI_Offset offset = D2D->rank_ * len;
    MPI_File file;

    // MPI to write in the binary file
    MPI_File_open(MPI_COMM_WORLD, filename, MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &file);
    MPI_File_write_at_all(file, offset, all_data, local_N_ * N_, MPI_DOUBLE, MPI_STATUS_IGNORE);
    MPI_File_close(&file);
...
//     int real_N_ = D2D->real_N_;
//     double *rho_ = D2D->rho_;
//     int procs = D2D->procs_;

//     zfp_field* field = zfp_field_1d(rho_, zfp_type_double, local_N_*N_);

//     if (!field) {
//         printf("Problem in memory allocation for field");
//     }

//     size_t compressed_size = zfp_field_size(field, NULL);
//     void* compressed_data = malloc(compressed_size);

//     zfp_stream* zfp = zfp_stream_open(NULL);

//     // Setting precision and rate as static 
//     uint precision = zfp_stream_precision(zfp);
//     double rate = zfp_stream_rate(zfp, 2);

//     // Setting compression stream
//     zfp_stream_set_bit_stream(zfp, compressed_data);
...
//     // Each process needs to know how it is going to write in the file:
//     // It needs to know how much data it is going to write 
//     MPI_Offset len = compressed_size * sizeof(size_t);
//     // It also needs to know where in the file to write
//     MPI_Offset offset = D2D->rank_ * len;
...
//     MPI_File_write_at_all(file, offset, compressed_data, compressed_size, MPI_BYTE, MPI_STATUS_IGNORE);
